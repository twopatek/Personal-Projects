...
Sys.setenv(
  OPENAI_API_KEY = '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'
)
```
...
general_ledger_accounts <- read_excel("testing_iber_jan_GL_file.xlsx") %>% 
  distinct(account)
...
results <- va_reference_accounts %>% 
  ...
  mutate(secondary_label = names[second_max_value_index]) %>%
  mutate(tertiary_label = names[third_max_value_index]) %>%
  select(-max_value_index, -second_max_value_index, -third_max_value_index) %>% 
  pivot_wider(id_cols = c("account", "primary_label", "secondary_label", "tertiary_label"), 
              names_from = "names", 
              values_from = "values")
...
d <- d %>% 
  ...
  mutate(GL_AccountLabel = if_else(str_detect("Blended Student Services Revenue"), ""))
